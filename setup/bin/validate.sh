...
echo "INFO - Validating cluster baseline"
find ./manifests/cluster -maxdepth 2 -type f -name '*.yaml' -print0 | while IFS= read -r -d $'\0' file;
  do
...
find ./manifests -type f -name $kustomize_config -print0 | while IFS= read -r -d $'\0' file;
  do
